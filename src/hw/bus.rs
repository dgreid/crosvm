...
impl Bus {
    ...
    pub fn insert(&mut self, device: Arc<Mutex<BusDevice>>, base: u64, len: u64) -> bool {
        if len == 0 {
            return false
        }
        ...
        self.devices.insert(BusRange(base, len), device).is_none()
    }
...
mod tests {
    ...
    fn bus_insert() {
        ...
        assert!(!bus.insert(dummy.clone(), 0x10, 0));
        assert!(bus.insert(dummy.clone(), 0x10, 0x10));
        assert!(!bus.insert(dummy.clone(), 0x10, 0x10));
        assert!(!bus.insert(dummy.clone(), 0x10, 0x15));
        assert!(!bus.insert(dummy.clone(), 0x12, 0x15));
        assert!(bus.insert(dummy.clone(), 0x20, 0x05));
        assert!(bus.insert(dummy.clone(), 0x25, 0x05));
        assert!(bus.insert(dummy.clone(), 0x0, 0x10));
    }

...
const DEFAULT_MODEM_STATUS: u8 = 0x20 | 0x10 | 0x80; // data set ready, clear to send, carrier detect
...
///
/// This can optionally write all output to a Write trait object.
pub struct Serial {
    interrupt_enable: u8,
...
impl Serial {
    ...
    /// Queues raw bytes for the serial port to receive.
    pub fn queue_bytes(&mut self, c: &[u8]) -> Result<()> {
        if !self.is_loop() {
    ...
    fn handle_write(&mut self, offset: u8, v: u8) -> Result<()> {
        match offset as u8 {
            ...
            SCR => self.scratch = v,
            _ => {
                println!("unknown write to: {}", offset);
            }
        }
        Ok(())
    }

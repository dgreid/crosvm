...
use super::*;
use hw::BusDevice;
use sys_util::{Result, EventFd, GuestMemory};
...
pub trait VirtioDevice {
    ...
    fn features(&self, page: u32) -> u32 { 0 }

    /// Acknowledges that this set of features should be enabled.
    fn ack_features(&mut self, page: u32, value: u32) { }

    /// Reads this device configuration space at `offset`.
    fn read_config(&self, offset: u64, &mut [u8]) { }

    /// Writes to this device configuration space at `offset`.
    fn write_config(&mut self, offset: u64, &[u8]) { }

    /// Activates this device for real usage.
    fn activate(&mut self, mem: GuestMemory, interrupt_evt: EventFd, status: Arc<AtomicUsize>, queues: Vec<Queue>, queue_evts: Vec<EventFd>);
...
/// Implements the [MMIO](http://docs.oasis-open.org/virtio/virtio/v1.0/cs04/virtio-v1.0-cs04.html#x1-1090002)
...
/// is written to.
///
/// Typically one page (4097 bytes) of MMIO address space will sufficient to handle this transport
...
impl MmioDevice {
    ...
    pub fn new(mem: GuestMemory, device: Box<VirtioDevice>) -> Result<MmioDevice> {
        ...
        let queues = device.queue_max_sizes().iter().map(|&s| Queue::new(s)).collect();
        Ok(MmioDevice {
            device: device,
...
impl BusDevice for MmioDevice {
    fn read(&mut self, offset: u64, data: &mut [u8]) {
        let v = match offset {
            ...
            0x010 => self.device.features(self.features_select) | if self.features_select == 1 { 0x1 } else { 0x0 },
            ...
            _ =>  {
                println!("unknown mmio read: 0x{:x}", offset);
                0
            },
        };
    ...
    fn write(&mut self, offset: u64, data: &[u8]) {
        ...
        match offset {
            ...
            0x064 => { self.interrupt_status.fetch_and(!(v as usize), Ordering::SeqCst); },
            0x070 => self.driver_status = v,
            0x080 => mut_q = self.with_queue_mut(|q| q.desc_table = (q.desc_table & !0xffffffff) | v as u64),
            0x084 => mut_q = self.with_queue_mut(|q| q.desc_table = (q.desc_table & 0xffffffff) | ((v as u64) << 32)),
            0x090 => mut_q = self.with_queue_mut(|q| q.avail_ring = (q.avail_ring & !0xffffffff) | v as u64),
            0x094 => mut_q = self.with_queue_mut(|q| q.avail_ring = (q.avail_ring & 0xffffffff) | ((v as u64) << 32)),
            0x0a0 => mut_q = self.with_queue_mut(|q| q.used_ring = (q.used_ring & !0xffffffff) | v as u64),
            0x0a4 => mut_q = self.with_queue_mut(|q| q.used_ring = (q.used_ring & 0xffffffff) | ((v as u64) << 32)),
        ...
        if !self.device_activated && self.is_driver_ready() {
            if let Some(interrupt_evt) = self.interrupt_evt.take() {
                if let Some(mem) = self.mem.take() {
                    self.device.activate(mem, interrupt_evt, self.interrupt_status.clone(), self.queues.clone(), self.queue_evts.split_off(0));

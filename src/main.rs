...
fn main() {
    let matches = App::new("crosvm")
        ...
        .subcommand(SubCommand::with_name("run")
                        .arg(Arg::with_name("disk")
                                 ...
                                 .multiple(true)
                                 .takes_value(true))
                        .arg(Arg::with_name("rwdisk")
                                 .short("r")
                                 .long("rwdisk")
                                 .value_name("FILE")
                                 .help("writable disk image")
                                 .multiple(true)
                                 .takes_value(true))
    ...
    match matches.subcommand() {
        ...
        ("run", Some(matches)) => {
            ...
            matches.values_of("disk")
                .map(|paths|
                    disks.extend(paths.map(|ref p| DiskOption { path: p, writable: false })));
            matches.values_of("rwdisk")
                .map(|paths|
                    disks.extend(paths.map(|ref p| DiskOption { path: p, writable: true })));
            let config = Config {
                disks: disks,

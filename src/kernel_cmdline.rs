...
/// A builder for a kernel command line string that validates the string as
/// its being built. A CString can be constructed from this directly using
/// CString::new.
pub struct Cmdline {
    line: String,
    capacity: usize,
...
impl Cmdline {
    /// Constructs an empty Cmdline with the given capacity, which includes
    /// the nul terminator. Capacity must be greater than 0.
    ...
    fn end_push(&mut self) {
        // We must always be under capacity to leave room for a nul
        // terminator.
    ...
    pub fn insert<T: AsRef<str>>(&mut self, key: T, val: T) -> Result<()> {
        ...
        try!(valid_element(k));
        try!(valid_element(v));
        try!(self.has_capacity(k.len() + v.len() + 1));

...
fn create_pcie_switch_hotplug_ports(
    ...
    if available_buses.len() < (port_count as usize) * 2 {
        return Err(anyhow!("not enough PCI buses for PCIe switch hotplug ports"));
    }
    ...
    for chunk in available_buses.chunks(2) {
        ...
        let upstream_port = Arc::new(Mutex::new(PcieUpstreamPort::new(
            0,
            upstream_bus,
            false,
        )));
        ...
        hp_stub
            .hotplug_buses
            .insert(downstream_bus, downstream_port as Arc<Mutex<dyn HotPlugBus>>);
    }
    Ok(())
}
...
fn handle_hotplug_block_command<V: VmArch, Vcpu: VcpuArch>(
    ...
    match block_cmd {
        vm_control::BlockControlCommand::AddBlock { path, read_only } => {
            handle_hotplug_block_add(
                linux,
                sys_allocator,
                add_control_tube,
                hotplug_manager,

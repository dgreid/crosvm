...
impl URingContext {
    ...
    ///
    /// Unlike [`add_writev`](Self::add_writev), the buffer is described by the submission queue
    /// entry itself, so there is no iovec array that has to be kept alive for the duration of the
    /// operation.
    ///
    /// # Safety
    ...
    ///
    /// Unlike [`add_readv`](Self::add_readv), the buffer is described by the submission queue
    /// entry itself, so there is no iovec array that has to be kept alive for the duration of the
    /// operation.
    ///
    /// # Safety

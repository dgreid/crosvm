...
impl Vm {
    ...
    pub fn add_device_memory(&mut self, guest_addr: GuestAddress, mem: MemoryMapping) -> Result<()> {
        ...
        // accessing memory as a pointer isn't safe, but it will only be passed
        // to the kernel below.
        let mem_addr = unsafe {mem.as_ptr() as u64};
...
mod tests {
    ...
    fn create_vm() {
        ...
        let gm = GuestMemory::new(&vec![(GuestAddress::new(0), 0x1000)]).unwrap();
    ...
    fn add_memory() {
        ...
        vm.add_device_memory(GuestAddress::new(0x1000), mem).unwrap();
    }

     #[test]
    fn overlap_memory() {
        let kvm = Kvm::new().unwrap();
        let gm = GuestMemory::new(&vec![(GuestAddress::new(0), 0x10000)]).unwrap();
        let mut vm = Vm::new(&kvm, gm).unwrap();
        let mem_size = 0x2000;
        let mem = MemoryMapping::new(mem_size).unwrap();
        assert!(vm.add_device_memory(GuestAddress::new(0x2000), mem).is_err());
    }
    ...
    fn get_memory() {
        ...
        let obj_addr = GuestAddress::new(0xf0);

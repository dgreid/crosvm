...
impl Minijail {
    ...
    pub fn parse_seccomp_filters(&mut self, path: &Path) -> Result<()> {
        let pathstring = path.as_os_str().to_str().ok_or(Error::InvalidPathString)?;
        let filename = CString::new(pathstring).map_err(|_| Error::InvalidPathString)?;
        unsafe {
    ...
    pub fn use_alt_syscall(&mut self, table_name: &str) -> Result<()> {
        let table_name_string = CString::new(table_name)
                .map_err(|_| Error::InvalidPathString)?;
        let ret = unsafe {
    ...
    pub fn enter_chroot(&mut self, dir: &Path) -> Result<()> {
        let pathstring = dir.as_os_str().to_str().ok_or(Error::InvalidPathString)?;
        let dirname = CString::new(pathstring).map_err(|_| Error::InvalidPathString)?;
    ...
    unsafe fn close_open_fds(&self, inheritable_fds: &[RawFd]) -> Result<()> {
        ...
        for entry in fs::read_dir(FD_PATH).map_err(|_| Error::ReadFdDir)? {
            ...
            let name = name_path.strip_prefix(FD_PATH)
                    .map_err(|_| Error::InvalidPathString)?;
            let name_str = name.to_str().ok_or(Error::InvalidPathString)?;
            let fd = <i32>::from_str(name_str).map_err(|_| Error::InvalidPathString)?;

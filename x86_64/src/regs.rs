...
pub enum Error {
    ...
    // Writing the GDT entries failed.
    WriteGDTFailure,
    WriteIDTFailure,
    WritePML4Address,
    WritePDPTEAddress,
...
fn write_idt_value(val: u64, guest_mem: &GuestMemory) -> Result<()> {
    let boot_idt_addr = GuestAddress::new(BOOT_IDT_OFFSET);
    guest_mem
...
fn setup_page_tables(mem: &GuestMemory, sregs: &mut kvm_sregs) -> Result<()> {
    ...
    let boot_pml4_addr = GuestAddress::new(0x9000);
    let boot_pdpte_addr = GuestAddress::new(0xa000);
...
pub fn setup_sregs(mem: &GuestMemory, vcpu: &kvm::Vcpu) -> Result<()> {
    let mut sregs: kvm_sregs = vcpu.get_sregs()
        .map_err(Error::SRegsIoctlFailed)?;
...
mod tests {
    ...
    fn create_guest_mem() -> GuestMemory {
        GuestMemory::new(&vec![(GuestAddress::new(0), 0x10000)]).unwrap()
    }

    fn read_u64(gm: &GuestMemory, offset: usize) -> u64 {
        let read_addr = GuestAddress::new(offset);

...
use super::kqueue_source::KqueueSource;
...
pub enum Error {
    ...
    #[error("An error creating the kqueue waiting context: {0}")]
...
// An IO source previously registered with a KqueueReactor. Used to initiate asynchronous IO with
...
impl<F: AsRawDescriptor> RegisteredSource<F> {
    pub(crate) fn new(raw: &Arc<RawExecutor<KqueueReactor>>, f: F) -> Result<Self> {
        ...
        // Try to set O_NONBLOCK, but ignore ENOTTY which occurs for kqueue fds
        // (which are already non-blocking by design).
        if let Err(e) = add_fd_flags(raw_fd, libc::O_NONBLOCK) {
            if e.errno() != libc::ENOTTY {
                return Err(Error::SettingNonBlocking(e));
            }
        }
...
/// `Reactor` that manages async IO work using kqueue.
...
impl Reactor for KqueueReactor {
            ...
            match op {
                OpStatus::Pending(mut data) => {
                    ...
                    if let Err(e) = self
                        ...
                        .delete(&base::Descriptor(data.file.as_raw_fd()))
                    {
                        warn!("Failed to remove file from kqueue: {}", e);
                    }
                }
                OpStatus::Completed => {}
                OpStatus::WakeEvent => {}
        ...
        for e in events.iter() {
            ...
            if let Some(op) = ops.get_mut(token) {
                let (file, waker) = match mem::replace(op, OpStatus::Completed) {
                    ...
                    OpStatus::WakeEvent => {
                        // Reset the wake event and put it back in the slab.
                        *op = OpStatus::WakeEvent;
                        if let Err(e) = self.wake_event.reset() {
                            warn!("Failed to clear wake event: {}", e);
                        }
                        continue;
                    }
                    OpStatus::Completed => {
                        // Shouldn't happen but put the status back to completed.
                        *op = OpStatus::Completed;
                        continue;
                    }
                };
                drop(ops);

                // Remove from the WaitContext now that the operation is complete.
                if let Err(e) = self.poll_ctx.delete(&base::Descriptor(file.as_raw_fd())) {
                    warn!("Failed to remove file from kqueue: {}", e);
                }
    ...
    fn new_source<F: AsRawDescriptor>(
        ...
        Ok(IoSource::Kqueue(KqueueSource::new(f, ex)?))
    }

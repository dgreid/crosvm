...
impl UringReactor {
    ...
    // Calls `add_op` with the raw descriptor of `source` and a freshly allocated operation token,
    // then records the operation so that the source file and `mem` outlive the kernel's access to
    // them. The ring lock is held across `add_op` so that the operation is always registered
    // before its completion can be reaped.
    fn submit_op<F>(
        &self,

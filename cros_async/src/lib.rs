...
//!
//! When building futures to be run in an `URingExecutor` framework, use the following helper
...
use uring_executor::{Result, URingExecutor};

/// Creates an empty URingExecutor that can have futures returning `()` added via
...
///    use cros_async::uring_executor::add_future;
...
pub fn empty_executor() -> Result<impl Executor> {
    URingExecutor::new(UnitFutures::new())
}
...
pub fn select2<F1: Future + Unpin, F2: Future + Unpin>(
    ...
    URingExecutor::new(select::Select2::new(f1, f2)).and_then(|mut f| f.run())
}
...
pub fn select3<F1: Future + Unpin, F2: Future + Unpin, F3: Future + Unpin>(
    ...
    URingExecutor::new(select::Select3::new(f1, f2, f3)).and_then(|mut f| f.run())
}
...
pub fn select4<F1: Future + Unpin, F2: Future + Unpin, F3: Future + Unpin, F4: Future + Unpin>(
    ...
    URingExecutor::new(select::Select4::new(f1, f2, f3, f4)).and_then(|mut f| f.run())
}
...
>(
    ...
    URingExecutor::new(select::Select5::new(f1, f2, f3, f4, f5)).and_then(|mut f| f.run())
}
...
pub fn complete2<F1: Future + Unpin, F2: Future + Unpin>(
    ...
    URingExecutor::new(complete::Complete2::new(f1, f2)).and_then(|mut f| f.run())
}
...
pub fn complete3<F1: Future + Unpin, F2: Future + Unpin, F3: Future + Unpin>(
    ...
    URingExecutor::new(complete::Complete3::new(f1, f2, f3)).and_then(|mut f| f.run())
}
...
pub fn complete4<F1: Future + Unpin, F2: Future + Unpin, F3: Future + Unpin, F4: Future + Unpin>(
    ...
    URingExecutor::new(complete::Complete4::new(f1, f2, f3, f4)).and_then(|mut f| f.run())
}
...
>(
    ...
    URingExecutor::new(complete::Complete5::new(f1, f2, f3, f4, f5)).and_then(|mut f| f.run())
}

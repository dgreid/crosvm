...
async fn handle_queue<F>(
    ...
    F: FnMut(GuestAddress) -> (),
{
    let mut queue_event = match EventAsync::new(queue_event) {
        Ok(e) => e,
        Err(e) => {
...
async fn handle_irq_resample(interrupt: Rc<RefCell<Interrupt>>) {
    let resample_evt = interrupt
        ...
        .try_clone()
        .unwrap();
    let resample_evt = EventAsync::new(resample_evt).unwrap();
    loop {
...
async fn wait_kill(kill_evt: Event) {
    let kill_evt = EventAsync::new(kill_evt).unwrap();
...
fn setup_stat_signal() -> std::result::Result<(Event, EventAsync), StatSignalError> {
    ...
    let stat_async_event = EventAsync::new(stat_read_event).map_err(StatSignalError::Poll)?;

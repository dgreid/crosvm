...
impl Worker {
    ...
    fn run(&mut self, mut queue_evts: Vec<EventFd>, kill_evt: EventFd) {
        ...
        const POLL_COMMAND_SOCKET: u32 = 3;
        const POLL_KILL: u32 = 4;
        ...
        'poll: loop {
            let tokens = match poller.poll(&[(POLL_INFLATE, &inflate_queue_evt),
                                             (POLL_DEFLATE, &deflate_queue_evt),
                                             (POLL_COMMAND_SOCKET,
                                                 &self.command_socket as &Pollable),
                                             (POLL_KILL, &kill_evt)]) {
                Ok(v) => v,
                Err(e) => {
                    ...
                    POLL_COMMAND_SOCKET => {
                        ...
                        if let Ok(count) = self.command_socket.recv(&mut buf) {
                            if count == 4 {
                                ...
                                if increment < 0 &&
                                   increment.abs() > num_pages {
                                    continue 'read_tokens;
                                }
                                self.config.num_pages
                                    .fetch_add(increment as usize, Ordering::Relaxed);
...
impl VirtioDevice for Balloon {
    fn keep_fds(&self) -> Vec<RawFd> {
        let mut fds = Vec::new();
        fds.push(self.command_socket.as_ref().unwrap().as_raw_fd());
        fds
    }
    ...
    fn read_config(&self, offset: u64, mut data: &mut [u8]) {
        ...
        (&mut config[0..]).write_u32::<LittleEndian>(num_pages).unwrap();
        (&mut config[4..]).write_u32::<LittleEndian>(actual_pages).unwrap();
    ...
    fn write_config(&mut self, offset: u64, mut data: &[u8]) {
        ...
        self.config.actual_pages.store(new_actual as usize, Ordering::Relaxed);
    }

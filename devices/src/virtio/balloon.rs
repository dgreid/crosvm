...
async fn handle_queue<F>(
    mem: &GuestMemory,
    mut queue: Queue,
    mut queue_evt: U64Source<EventFd>,
...
{
    loop {
        let avail_desc = match queue.next_async(mem, &mut queue_evt).await {
            Err(e) => {
...
async fn handle_stats_queue(
    ...
    loop {
        let stats_desc = match queue.next_async(mem, &mut queue_evt).await {
            Err(e) => {
                error!("Failed to read descriptor {}", e);
                return;
            }
...
fn run_worker(
    ...
    let inflate_event = match U64Source::new(queue_evts.remove(0)) {
        Ok(e) => e,
        Err(e) => {
    ...
    let deflate_event = match U64Source::new(queue_evts.remove(0)) {
        Ok(e) => e,
        Err(e) => {
    ...
    let stat_event = match U64Source::new(queue_evts.remove(0)) {
        Ok(e) => e,
        Err(e) => {
    ...
    let stat_async_event = match U64Source::new(stat_read_event) {
        Ok(e) => e,
        Err(e) => {
            error!("Balloon Failed to set up the stats signal event: {}", e);
            return;

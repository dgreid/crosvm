    ...
    loop {
        ...
        let (hdr, files) = match backend_server.recv_header() {
            ...
            Err(VhostError::ClientExit) => {
                ...
                // TODO(b/0): On macOS, vhost-user backends may run in-process rather than
                // as separate processes. If so, process::exit(0) would terminate the entire
                // VMM. This is inherited from the Linux handler where backends are always
                // separate processes.

...
use base::{error, info, iov_max, warn, Event, Timer};
use cros_async::{select4, AsyncError, EventAsync};
...
async fn flush_timer(disk_state: Rc<RefCell<DiskState>>) -> result::Result<(), OtherError> {
    ...
    let async_timer = EventAsync::new(timer).map_err(OtherError::AsyncTimerCreate)?;
...
async fn handle_irq_resample(interrupt: Rc<RefCell<Interrupt>>) -> result::Result<(), OtherError> {
    ...
    let resample_evt = EventAsync::new(resample_evt).map_err(OtherError::AsyncResampleCreate)?;
    loop {
        let _ = resample_evt
            .next_val()
            .await
...
async fn wait_kill(kill_evt: Event) -> result::Result<(), OtherError> {
    let kill_evt = EventAsync::new(kill_evt).map_err(OtherError::AsyncKillCreate)?;
...
fn run_worker(
    ...
    let queue_handlers = queues
        ...
        .zip(
            queue_evts
                .into_iter()
                .map(|e| EventAsync::new(e).expect("Failed to create async event for queue")),
        )
        .map(|(queue, event)| {

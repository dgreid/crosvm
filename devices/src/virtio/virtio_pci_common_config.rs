...
mod tests {
    ...
    fn write_base_regs() {
        ...
        regs.write(0x20, &[0x55], &mut queues, &mut dev);
        let mut read_back = vec![0x00];
        regs.read(0x20, &mut read_back, &mut queues, &mut dev);
        assert_eq!(read_back[0], 0x55);

        // The config generation register is read only.
        regs.write(0x21, &[0xaa], &mut queues, &mut dev);
        let mut read_back = vec![0x00];
        regs.read(0x21, &mut read_back, &mut queues, &mut dev);
        assert_eq!(read_back[0], 0x55);

...
/// Get the maximum number of segments for a request.
/// On macOS, we don't have IOV_MAX readily available via libc,
/// so we use a reasonable default.
pub fn get_seg_max(queue_size: u16) -> u32 {
    // Use a reasonable default for macOS (similar to Linux's typical IOV_MAX)
    let seg_max = 1024u32;
...
impl DiskOption {
    ...
    pub fn open(&self) -> anyhow::Result<Box<dyn DiskFile>> {
        disk::open_disk_file(disk::DiskFileParams {
            ...
            is_overlapped: false,
            // O_DIRECT is not supported on macOS, so we ignore this flag
            is_direct: false,
            lock: self.lock,
            depth: 0,
        })

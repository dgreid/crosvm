...
impl RefCount {
    ...
    fn check_evict(&mut self, raw_file: &mut QcowRawFile, except: usize) -> io::Result<()> {
        ...
        if self.refblock_cache.len() == self.refblock_cache.capacity() {
            ...
            if let Some(evicted) = self.refblock_cache.remove(&to_evict) {
                if evicted.dirty() {
                    raw_file.write_refcount_block(self.ref_table[to_evict], evicted.get_vec())?;
                }
            }
        }
        Ok(())
    ...
    pub fn set_cluster_refcount(
        ...
        if let Entry::Vacant(e) = self.refblock_cache.entry(table_index) {
            // Need a new cluster
            if block_addr_disk == 0 {
                if let Some((addr, table)) = new_cluster.take() {
                    ...
                    return Err(Error::NeedNewCluster(0));
                }
            } else {
                if let Some((addr, table)) = new_cluster.take() {
                    if addr != block_addr_disk {
        ...
        self.check_evict(raw_file, table_index).map_err(Error::EvictingRefCounts)?;
    ...
    pub fn flush_blocks(&mut self, raw_file: &mut QcowRawFile) -> io::Result<()> {
        ...
        for (table_index, block) in self.refblock_cache.iter_mut().filter(|(_k, v)| v.dirty()) {
            ...
            if addr != 0 {
                raw_file.write_refcount_block(addr, block.get_vec())?;
            } else {
        ...
        let refcount = match self.refblock_cache.entry(table_index) {
            Entry::Vacant(e) => {
                let table = VecCache::from_vec(
                    raw_file.read_refcount_block(block_addr_disk)
    ...
    pub fn refcount_block(&mut self, raw_file: &mut QcowRawFile, table_index: usize) -> Result<Option<&[u16]>> {
        let block_addr_disk = *self.ref_table.get(table_index).ok_or(Error::InvalidIndex)?;
        if block_addr_disk == 0 {
            return Ok(None);
        }
        ...
        Ok(Some(self.refblock_cache.get(&table_index).unwrap().get_vec()))
    }

...
impl QcowHeader {
    ...
    pub fn write_to<F: Write + Seek>(&self, file: &mut F) -> Result<()> {
        ...
        // to a `File` instead of anything that implements seek as the `file` argument.
        // This zeros out the l1 and refcount table clusters.
...
impl Read for QcowFile {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        self.dump_action(ActionType::Read(buf.len() as u64));

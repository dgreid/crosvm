...
impl QcowFile {
    ...
    pub fn from(mut file: File) -> Result<QcowFile> {
        ...
        let mut raw_file = QcowRawFile::from(
                file,
                cluster_size,
                cluster_size - 1,
        );
        ...
        let refcounts = RefCount::new(&mut raw_file, header.refcount_table_offset, 
                                      refcount_clusters, refcount_block_entries, cluster_size)
    ...
    pub fn first_zero_refcount(&mut self) -> Result<Option<u64>> {
        let file_size = self.raw_file.file_mut().metadata().map_err(Error::GettingFileSize)?.len();
        let cluster_size = 0x01u64 << self.header.cluster_bits;

        let mut cluster_addr = 0;
        while cluster_addr < file_size {
            match self.refcounts
    ...
    fn check_l2_evict(&mut self, except: usize) -> std::io::Result<()> {
        ...
        if self.l2_cache.len() == self.l2_cache.capacity() {
            ...
            if let Some(evicted) = self.l2_cache.remove(&to_evict) {
                if evicted.dirty() {
                    self.write_l2_cluster(to_evict, evicted.get_vec())?;
                }
            }
        }
        Ok(())
    ...
    fn set_cluster_refcount(&mut self, address: u64, refcount: u16) -> std::io::Result<Vec<u64>> {
        ...
        while !refcount_set {
            match self.refcounts.set_cluster_refcount(&mut self.raw_file, address, refcount,
                                                      new_cluster.take()) {
                Ok(None) => {
                    refcount_set = true;
                },
    ...
    fn sync_caches(&mut self) -> std::io::Result<()> {
        ...
        for (l1_index, l2_table) in self.l2_cache.iter_mut().filter(|(_k, v)| v.dirty())
        {
            ...
            let addr = self.l1_table[*l1_index];
            if addr != 0 {
                self.raw_file.write_pointer_table(addr, l2_table.get_vec(), CLUSTER_USED_FLAG)?;
            } else {
                return Err(std::io::Error::from_raw_os_error(EINVAL));
            }
            l2_table.mark_clean();
    ...
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        while nread < read_count {
            ...
            if let Some(offset) = file_offset {
                ...
                self.raw_file.file_mut().read_exact(&mut buf[nread..(nread + count)])?;
            } else {
...
impl Write for QcowFile {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        while nwritten < write_count {
            ...
            if let Err(e) = self.raw_file.file_mut().write(&buf[nwritten..(nwritten + count)]) {
                return Err(e);
            }

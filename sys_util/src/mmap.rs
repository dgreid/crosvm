...
impl MemoryMapping {
    ...
    /// Returns Ok(<number of bytes written>).  Then number of bytes written can
    ...
    pub fn write_slice(&self, buf: &[u8], offset: usize) -> Result<usize> {
        unsafe {
            ...
            if offset >= self.size {
                return errno_result(); // TODO(dgreid)
            }
        ...
        where F: Read
    {
        ...
        if mem_end > self.size() {
            return errno_result(); // TODO(dgreid) Err(Error::ImagePastRamEnd);
        }
        unsafe {
            ...
            if src.read_exact(dst).is_err() {
                // TODO(dgreid) error type.
                return errno_result(); // TODO(dgreid) Err(Error::ReadFromSource);
            }
        }
        Ok(())
    }
        ...
        where F: Write
    {
        ...
        unsafe {
            ...
            // memory as a mutable slice is OK because nothing assumes another
            // thread won't change what is loaded.
            let src = &mut self.as_mut_slice()[mem_offset..mem_end];

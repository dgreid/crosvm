...
fn modify_mode<F: FnOnce(&mut termios)>(fd: RawFd, f: F) -> Result<()> {
    // SAFETY:
    // Safe because passing any int to isatty has defined behavior, invalid fd's will be ignored.
    ...
    let mut termios: termios = unsafe {
        let mut t = zeroed();
        let ret = tcgetattr(fd, &mut t as *mut _);
        if ret < 0 {
            return errno_result();
        }
        t
    ...
    f(&mut new_termios);
    // SAFETY:
    // Safe because the syscall will only read the extent of termios.
...
pub fn read_raw_stdin(out: &mut [u8]) -> Result<usize> {
    // SAFETY:
    // Safe because it is reading from stdin to a valid buffer.

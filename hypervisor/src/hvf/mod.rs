...
impl Vm for HvfVm {
    ...
    fn remove_mapping(&self, _slot: u32, _offset: usize, _size: usize) -> Result<()> {
        Err(Error::new(ENOTSUP))
    }

    fn handle_balloon_event(&self, _event: BalloonEvent) -> Result<()> {
        Ok(())
    }
...
impl VmAArch64 for HvfVm {
    ...
    fn create_vcpu(&self, id: usize) -> Result<Arc<dyn VcpuAArch64>> {
        Ok(Arc::new(HvfVm::create_vcpu(self, id)?))
    }
